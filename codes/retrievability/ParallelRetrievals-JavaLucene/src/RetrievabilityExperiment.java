...
public class RetrievabilityExperiment {
    ...
    private static final List<String> querySetFiles = Collections.singletonList("../../Query-Sets/artificial-all-queries.json");
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        try (IndexReader indexReader = DirectoryReader.open(FSDirectory.open(Paths.get(indexPath)))) {
            ...
            if (forkJoinPool.isTerminated()) {
                try (FileWriter writer = new FileWriter("../../for-Artificial/rd_dump/allrd_counters.json")) {
                    Gson gson = new Gson();
                    gson.toJson(allRd, writer);
                    System.out.println("Results saved to file: for-Artificial/rd_dump/allrd_counters.json");
                } catch (IOException e) {
